...
function tabloyuOlustur() {
    ...
    for(let i=0; i<7; i++) {
        if(haftalikProgram["BARIŞ İNCE"][i] !== "00:00–07:00") haftalikProgram["EKREM FİDAN"][i] = "00:00–07:00";
    }
    ...
    // MCR Rotasyon (2Sabah 2Akşam 2Gece 2İzin)
    applyMCRRota("24TV MCR OPERATÖRÜ");
    applyMCRRota("360TV MCR OPERATÖRÜ");

    // Playout & KJ (Değişken 09:00)
    const pSor = setDegisken("Playout Operatörü");
    const kSor = setDegisken("KJ Operatörü");

    // İzin Dağıtımı (Kısıtlı olmayanlar için)
    personeller.forEach(p => {
        if(["BARIŞ İNCE", "ZAFER AKAR", pSor, kSor].includes(p.isim) || p.birim.includes("MCR")) return;
        let c = 0;
        while(c < 2) {
            let r = Math.floor(Math.random() * 7);
...
function applyMCRRota(birim) {
    ...
    ekip.forEach((p, idx) => {
        for(let i=0; i<7; i++) {
            let gun = new Date(mevcutPazartesi.getTime() + (i * 86400000));
            let fark = Math.floor((gun - ref) / 86400000);
            let rIdx = (fark + (idx * 2)) % 8;
            if(rIdx < 0) rIdx += 8;
            haftalikProgram[p.isim][i] = rota[rIdx];
        }
    });
}
...
function hucreDoldur(gun, saat) {
    ...
    birimSiralamasi.forEach(birim => {
        ...
        if(birim === "Teknik Yönetmen") kap = !isHS ? (saat === "06:30–16:00" ? 2 : (saat === "16:00–00:00" ? 1 : 0)) : 1;
        else if(birim === "Ses Operatörü") kap = !isHS ? (saat === "06:30–16:00" ? 4 : (saat === "16:00–00:00" ? 2 : 0)) : 2;
        else if(birim.includes("Playout") || birim.includes("KJ")) kap = 2;
...
function ozetTablosuGuncelle() {
    let h = `<table class="stats-table"><thead><tr><th>Personel İsmi</th><th>Birim</th><th>Toplam Mesai</th><th>Gece Vardiyası</th></tr></thead><tbody>`;
    personeller.forEach(p => {
        ...
        const tehlikeSinifi = mesai >= 6 ? "tehlike-mesai" : "";
        h += `<tr>
            <td><strong>${p.isim}</strong></td>
            <td><small>${p.birim}</small></td>
            <td class="${tehlikeSinifi}">${mesai} Gün</td>
            <td class="gece-hucre">${gece} Gece</td>
        </tr>`;
    });
